...
class CanvasFrame extends React.Component {
    ...
    controlActions(...args) {
        ...
        const undo = ()       => { return; } // TODO: Anything
        ...
        const flipY = ()      => { 
            let newRows = this.state.pixelRows.reverse() 
            return apply(newRows); 
        }
        ...
        const clear = ()      => { return apply(Array(8).fill(0)); } 
        ...
        return (
            <div className="frame">
                ...
                <div className="frame-container">
                    {Object.keys(this.controlActions().shift).map(dir => {
                        return (
                            <FrameShifter 
                                key={"shift-" + dir}
                                dir={dir}
                                action={this.controlActions().shift[dir].func}
                                icon={this.controlActions().shift[dir].icon} 
                            />
                        );
                    })}
                    {this.renderCanvas()}
    ...
    render() {
        return (
           <div className="frame-controls">
                {Object.keys(this.props.actions.modify).map(act => {
                    // Output buttons for all the actions from the bound function handler
                    let action = this.props.actions.modify[act];
                    return (
                        <Button 
                            key={"action-" + act} 
                            action={action.func} 
                            buttonClass="frame-control" 
                            tooltip={action.name} 
                            text={action.name}
                        />
                    )})
...
class FrameShifter extends React.Component {
    ...
    render() {
        return (
            <Button icon={this.props.icon} buttonClass={"frame-shift " + this.props.dir} action={this.props.action} text={this.props.label} />
        );
    }
